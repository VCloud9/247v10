    ...
    def check_jobclass(self):
        ...
        for record in self:
            if record.report_id.pv_site:
                ...
                rate_obj = self.env['job.prevailing.wage'].search(['|',('start_date','<=',today),('start_date','=',False), \
    ...
    def action_approve(self):
        ...
        for line in self.employee_hours:
            time_line.create({
                ...
                'task_id': task_id.id,
                'name': 'Regular Hours',
                'rate': line.employee_standard_rate,
                'unit_amount': line.total_regular_hours,
            })
            if line.total_passenger_hours > 0:
                time_line.create({
                'date': self.date,
                'user_id': line.employee_id.user_id.id,
                'project_id': self.job_id.id,
                ...
                'rate': line.passenger_cost,
                'unit_amount': line.total_passenger_hours,
                })
            if line.total_drive_hours > 0:
                if self.pv_site:
                    drive_cost = 0
                else:
                    drive_cost = line.drive_cost
                time_line.create({
                'date': self.date,
                'user_id': line.employee_id.user_id.id,
                'project_id': self.job_id.id,
                'task_id': task_id.id,
                'name': 'Driving Hours',
                'rate': line.drive_cost,
                'unit_amount': line.total_drive_hours,
                })
            if line.total_pw_hours > 0:
                time_line.create({
                'date': self.date,
                'user_id': line.employee_id.user_id.id,
                'project_id': self.job_id.id,
                'task_id': task_id.id,
                'name': 'Prevailing Wage Hours',
                'rate': line.pw_cost,
                'unit_amount': line.total_pw_hours,
                })
